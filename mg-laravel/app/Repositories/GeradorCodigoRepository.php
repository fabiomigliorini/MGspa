...
class GeradorCodigoRepository {
    ...
    public static function caminhoPolicy ($arquivo = '') {
        if (!empty($arquivo)) {
    ...
    public static function caminhoViewIndex ($url) {
        return static::caminhoDirView($url) . '/index.blade.php';
    }

    public static function caminhoViewShow ($url) {
        return static::caminhoDirView($url) . '/show.blade.php';
    }

    public static function caminhoViewCreate ($url) {
        return static::caminhoDirView($url) . '/create.blade.php';
    }

    public static function caminhoViewEdit ($url) {
        return static::caminhoDirView($url) . '/edit.blade.php';
    }

    public static function caminhoViewForm ($url) {
        return static::caminhoDirView($url) . '/form.blade.php';
    }

    public static function caminhoDirView ($url) {
        return base_path() . '/resources/views/' . $url;
    }

    public static function criaCaminhoDirView($url) {
        $pasta = static::caminhoDirView($url);
        if (is_dir($pasta)) {
            return true;
        }
        if (!mkdir($pasta)) {
    ...
    public static function buscaTabelas() {
    ...
    public static function buscaCamposListagem ($tabela, $instancia_model, $coluna_titulo) {
    ...
    public static function montaCamposForm($cols, $instancia_model) {
    ...
    public static function geraPolicy($model) {
        return view('gerador-codigo.arquivos.policy', compact('model'))->render();
    }

    public static function geraViewIndex($tabela, $model, $titulo, $url, $coluna_titulo) {
        $instancia_model = "\\App\\Models\\{$model}";
        $instancia_model = new $instancia_model();
        $cols_listagem = static::buscaCamposListagem($tabela, $instancia_model, $coluna_titulo);
    ...
    public static function geraViewShow($tabela, $model, $titulo, $url, $coluna_titulo) {
        $instancia_model = "\\App\\Models\\{$model}";
        $instancia_model = new $instancia_model();
        $cols_listagem = static::buscaCamposListagem($tabela, $instancia_model, $coluna_titulo);
    ...
    public static function geraViewCreate($tabela, $model, $titulo, $url, $coluna_titulo) {
        return view('gerador-codigo.arquivos.view-create', compact('url'))->render();
    }

    public static function geraViewEdit($tabela, $model, $titulo, $url, $coluna_titulo) {
        $instancia_model = "\\App\\Models\\{$model}";
        $instancia_model = new $instancia_model();
        return view('gerador-codigo.arquivos.view-edit', compact('instancia_model', 'model', 'url'))->render();
    }

    public static function geraViewForm($tabela, $model, $titulo, $url, $coluna_titulo) {
        $instancia_model = "\\App\\Models\\{$model}";
        $instancia_model = new $instancia_model();
        $cols = static::buscaCamposTabela($tabela);
        $cols = static::montaCamposForm($cols, $instancia_model);
        return view('gerador-codigo.arquivos.view-form', compact('tabela', 'model', 'titulo', 'url', 'instancia_model', 'coluna_titulo', 'cols'))->render();
    }

    public static function salvaPolicy($model) {
        $conteudo = static::geraPolicy($model);
        $arquivo = static::caminhoPolicy($model);
        return (static::salvaArquivo($arquivo, $conteudo));
    }

    public static function salvaViewIndex($tabela, $model, $titulo, $url, $coluna_titulo) {
        $conteudo = static::geraViewIndex($tabela, $model, $titulo, $url, $coluna_titulo);
        $arquivo = static::caminhoViewIndex($url);
        static::criaCaminhoDirView($url);
        return (static::salvaArquivo($arquivo, $conteudo));
    }

    public static function salvaViewShow($tabela, $model, $titulo, $url, $coluna_titulo) {
        $conteudo = static::geraViewShow($tabela, $model, $titulo, $url, $coluna_titulo);
        $arquivo = static::caminhoViewShow($url);
        static::criaCaminhoDirView($url);
        return (static::salvaArquivo($arquivo, $conteudo));
    }

    public static function salvaViewCreate($tabela, $model, $titulo, $url, $coluna_titulo) {
        $conteudo = static::geraViewCreate($tabela, $model, $titulo, $url, $coluna_titulo);
        $arquivo = static::caminhoViewCreate($url);
        static::criaCaminhoDirView($url);
        return (static::salvaArquivo($arquivo, $conteudo));
    }

    public static function salvaViewEdit($tabela, $model, $titulo, $url, $coluna_titulo) {
        $conteudo = static::geraViewEdit($tabela, $model, $titulo, $url, $coluna_titulo);
        $arquivo = static::caminhoViewEdit($url);
        static::criaCaminhoDirView($url);
        return (static::salvaArquivo($arquivo, $conteudo));
    }

    public static function salvaViewForm($tabela, $model, $titulo, $url, $coluna_titulo) {
        $conteudo = static::geraViewForm($tabela, $model, $titulo, $url, $coluna_titulo);
        $arquivo = static::caminhoViewForm($url);
        static::criaCaminhoDirView($url);
        return (static::salvaArquivo($arquivo, $conteudo));
    }

    public static function stringRegistroPolicy($model) {
        return "\\App\\Models\\{$model}::class => \\App\\Policies\\{$model}Policy::class";
    }

    public static function verificaRegistroPolicy($model) {
