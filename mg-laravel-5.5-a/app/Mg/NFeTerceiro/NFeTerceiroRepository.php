...
class NFeTerceiroRepository extends MgRepository
{
        ...
        foreach ($qry as $key => $file) {
            ...
            if(file_exists($path)){
                ...
                if($file->schema == 'resNFe_v1.01.xsd'){
                  ...
                  $pessoa = static::novaPessoa($uf, $cnpj, $ie, $cpf, $filial);
    ...
    public static function novaPessoa($uf, $cnpj, $iest, $cpf, $filial){

      // BUSCA NA BASE DE DADOS O codpessoa
      $pessoa = Pessoa::where([['ie', $iest],['cnpj', $cnpj]] )->orWhere([['ie', $iest],['cnpj', $cpf]])->first();
      ...
      if($pessoa == null){
        try {
          ...
          $response = $tools->sefazCadastro('SC', $cnpj, $iest, $cpf);
