...
const changeStatus = async (statusValue) => {
  ...
  if (ok) {
    Notify.create({
      type: 'positive',
      message: 'Pedido ' + sWoo.pedido.id + ' teve o status alterado para ' + statusValue + '.',
      timeout: 3000,
  ...
  <q-dialog v-model="show" transition-show="scale" transition-hide="scale">
    <q-card style="width: 500px; max-width: 90vw">
      <q-card-section class="q-pa-md">
        <div class="text-grey-8 q-mb-md">Selecione o novo status para este pedido:</div>
        ...
        <div class="row q-col-gutter-sm">
          <div v-for="opt in statusOptions" :key="opt.value" class="col-6">
            <q-item
              ...
              class="full-height"
              :class="sWoo.pedido.status === opt.value ? 'bg-blue-2' : 'bg-grey-3'"
            >
