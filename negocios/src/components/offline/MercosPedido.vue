        ...
        <q-item>
          <q-item-section avatar top>
            <template v-if="sNegocio.negocio.codnegociostatus == 2">
              <q-btn
                @click="informarFaturamento(mp)"
                round
                :color="colorIconUpload(mp)"
                icon="mdi-cloud-upload-outline"
              />
            </template>
            <template v-else>
              <q-btn
                @click="confirmarReimportar(mp)"
                round
                color="secondary"
          ...
          <q-item-section>
            <q-item-label lines="1"> Mercos #{{ mp.numero }} </q-item-label>
