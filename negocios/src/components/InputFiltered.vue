...
import { primeiraLetraMaiuscula } from "src/utils/formatador.js";
...
<template>
  <q-input ref="refInput" v-model="model" @keyup="aplicarFiltro" @focus="modelAnterior = model">
    <template v-slot:append>
      <q-icon :name="filtrar ? 'published_with_changes' : 'unpublished'" class="cursor-pointer"
        @click="toggleFiltrar()" />
      <slot name="append" />
    </template>
  </q-input>
