...
const selecionarAba = (sel) => {
  ...
  switch (selecionado.value.substring(0, 3)) {
    ...
    case 'CAT':
      ...
      categoria.value = localizaCategoria(codpranchetacategoria.value, prancheta.value)
...
const adicionarProduto = () => {
  Dialog.create({
    ...
    setTimeout(() => {
      refTree.value.setExpanded('CAT' + categoria.value.codpranchetacategoria, true)
    }, 750)
    corrigirOrdem()
  })
}
...
const removerCategoria = () => {
  Dialog.create({
    ...
    if (categoria.value.codpranchetacategoriapai == null) {
      ...
      let pai = localizaCategoria(categoria.value.codpranchetacategoriapai, prancheta.value)
...
const alterarCategoriaProduto = (codpranchetacategorianova) => {
  ...
  let novaCategoria = localizaCategoria(codpranchetacategorianova, prancheta.value)

  // coloca por ultimo na ordem
  produto.value.ordem = 1
  if (novaCategoria.produtos.length > 0) {
    produto.value.ordem = Math.max(...novaCategoria.produtos.map((prd) => prd.ordem)) + 1
  }
  ...
  let antigaCategoria = localizaCategoria(codpranchetacategoriapai.value, prancheta.value)
...
const alterarCategoriaPai = (codpranchetacategorianova) => {
  ...
  if (codpranchetacategorianova == null) {
    ...
    if (prancheta.value.length > 0) {
      categoria.value.ordem = Math.max(...prancheta.value.map((cat) => cat.ordem)) + 1
    }
    ...
    if (novaCategoria.categorias.length > 0) {
      categoria.value.ordem = Math.max(...novaCategoria.categorias.map((cat) => cat.ordem)) + 1
    }
      ...
      <q-card class="q-ma-md col-xs-11 col-sm-5 col-md-4 col-lg-3 col-xl-2">
                    ...
                    <div class="col" style="max-width: 350px">
                      <q-form class="q-gutter-md" v-if="categoria">
                        ...
                        <q-btn-group flat>
                          ...
                          >
                            <q-tooltip color="secondary"> Para Baixo </q-tooltip>
                          </q-btn>
                          <q-btn flat color="negative" icon="delete" @click="removerCategoria()">
                            <q-tooltip color="secondary"> Excluir Categoria </q-tooltip>
                          </q-btn>
                          <q-btn-dropdown flat auto-close color="primary" icon="add">
                            ...
                            <q-item clickable @click="adicionarProduto()">
                              <q-item-section>
                                <q-item-label class="text-primary"> Novo Produto </q-item-label>
                              </q-item-section>
                            </q-item>
                          </q-btn-dropdown>
                        </q-btn-group>
                    ...
                    <div class="col" style="max-width: 350px">
                      <q-form v-if="produto" class="q-gutter-md">
                        ...
                        <q-btn-group flat>
                          ...
                          <q-btn flat color="negative" icon="delete" @click="removerProduto()">
                          ...
                          <q-btn
                            ...
                            target="_blank"
                          >
                            <q-tooltip> Ir para o cadastro do Produto </q-tooltip>
                          </q-btn>
                        </q-btn-group>
                      </q-form>
                    </div>
        ...
        <q-card-actions>
          <q-btn
            ...
            :disable="rodando"
          />
          <q-btn flat label="Salvar" color="primary" @click="salvar()" :disable="rodando" />
        </q-card-actions>
      </q-card>
    </div>
  </q-page>
