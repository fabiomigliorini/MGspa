...
FALSO ALARME — engano meu, nao bug do sistema. O sintoma (curl/Chrome recebendo resposta HTTP vazia em ~0ms nas portas 8088/8081/8082/9900) era eu testando com http:// puro contra um dev server que so fala HTTPS (devServer.https=true, ja documentado na TASK-111: certificado self-signed do @quasar/ssl-certificate). Um servidor TLS recebendo bytes de HTTP em texto plano fecha a conexao sem responder nada — exatamente o ERR_EMPTY_RESPONSE/'Empty reply from server' que eu vi. Confirmado em 24/09: curl -k https://127.0.0.1:8088/ e :8081/ voltam 200 na hora, e o Chrome headless com --ignore-certificate-errors renderiza normalmente a tela de login em https://sistema-dev.mgpapelaria.com.br:8088/ (screenshot ok). Os 4 dev servers estavam saudaveis o tempo todo; nao ha bug de infra aqui. Fechando.
