...
class CargoService
{
    public static function index($cargo, $de, $ate)
    {
        ...
        if (!empty($cargo)) {
            $sql .= ' where ca.cargo ilike :cargo ';
            $params['cargo'] = "%{$cargo}%";
        }

        if (!empty($de)) {
            $sql .= ' and ca.salario >= :de ';
            $params['de'] = $de;
        }

        if (!empty($ate)) {
            $sql .= ' and ca.salario <= :ate ';
            $params['ate'] = $ate;
        }

        $sql .= ' order by ca.cargo asc ';

        return $params ? DB::select($sql, $params) : DB::select($sql);
    }

    public static function create($data): Cargo
    {
    ...
    public static function pessoasDoCargo($codcargo)
    {
        ...
        return DB::select($sql, ['codcargo' => $codcargo]);
    }
}
