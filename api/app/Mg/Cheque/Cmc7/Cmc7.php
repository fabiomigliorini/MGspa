...
namespace Mg\Cheque\Cmc7;

/**
 * Parse e validação da linha CMC7 do cheque.
 *
 * Porta literal de /opt/www/MGLara/app/Library/Cmc7/Cmc7.php
 * (apenas namespace ajustado para Mg\Cheque\Cmc7).
 *
 * @property string $cmc7 Número do CMC7 (apenas dígitos)
 */
class Cmc7
{
    protected $cmc7;
    ...
    public function DigitosaIgnorarContaCMC7()
    {
        ...
        if ($this->banco() == 41) {
            $digitostirar = 0; // 041 - Banrisul (usa todo o campo para o número da conta)
        }
        if ($this->banco() == 104) {
            $digitostirar = 0; // 104 - CEF (usa apenas 7, mas os 3 primeiros são necessários para o dv)
        }
        if ($this->banco() == 237) {
