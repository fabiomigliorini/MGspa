...
> **Status:** ✅ O novo projeto Laravel 13 está em pé, com **paridade total de auth com o MGAuth** (Marco 1) **+ 44 controllers já migradas** do MGspa/laravel (Marco 3 — 34% de 128) **+ proxy fallback ativo** pras rotas ainda não migradas. **Login real validado e testado pelo usuário no frontend Quasar `pessoas`.** 178 rotas v1 nativas, todos os 42 endpoints CRUDs principais respondem 200/206.
...
### Controllers migradas (Marco 3) — 44 controllers, 178 rotas v1
...
|---|---|---|
| ✅ Migrado | 44 | 34% |
| ⏭️ Pulado / adiado por dep complexa | 5 | 4% |
| 🟢 Leve restante | ~8 | 6% |
| 🟡 Médio | ~38 | 30% |
| 🔴 Pesado/integração externa | ~33 | 26% |
| **Total** | **128** | **100%** |

### Próximas candidatas (mais baratas / impacto maior)

**🟢 Leves ainda não migrados:**
- `Portador` (199 linhas, mas usa `App\Mg\Portador\ExtratoBbService` externo — verificar)
- `Marca` (187 linhas, depende de Imagem)
- `Etiqueta` (1 rota GET utilitária, restante depende de Produto/Negocio)

**🟡 Domínios completos médios** (1-2 dias cada):
- **Tributacao trio** (Tributacao + TributacaoRegra + Tributo) — apiResource, mas TributacaoService puxa NotaFiscalItemTributo
- **NaturezaOperacao trio** (Cfop + NaturezaOperacao + TributacaoNaturezaOperacao) — sub-namespaces Resources/Requests, médio
- **Cidade** (Pais + Estado + Cidade, nested complexo + Resources Detail vs List)
- **Rh** (Acerto, ColaboradorRubrica, Indicador, MeuPainel, PeriodoColaborador, PeriodoColaboradorSetor, Periodo) — 7 controllers, depende de Colaborador
- **Pessoa nested restante**: PessoaTelefone (SMS verification), PessoaEmail (verification), PessoaEndereco (depende `PessoaService::atualizaCamposLegado`)
- **Filial core** (médio, várias deps)
- **Usuario + GrupoUsuario** (gerência de permissões)
- **Pedido** (médio)
- **CaixaMercadoria**, **Dominio**, **GrupoEconomico** (com 5 endpoints especiais)
...
- `NotaFiscalController` + 7 subs (30+ rotas, NFe)
- `PdvController` + 4 subs (30+ rotas, PDV)
...
- `ColaboradorController` + `ColaboradorCargoController` + `FeriasController` + `ColaboradorFichaController` (core RH com upload)
- `BoletoBb\*`, `TituloController`, `TituloRelatorio` (financeiro core)
- `Produto`, `NfeTerceiro`, `Banco\BoletoController`
