<template>
  <q-card class="no-shadow" bordered>
    ...
    <q-card-section class="q-pa-none">
      <q-table square class="no-shadow" title="Notas Por Emitir" @row-click="linksnotasemitir" :rows="arraylistanotas"
        :columns="columns" :loading="loading" no-data-label="Nenhuma nota por Emitir encontrada." :filter="filter"
...
export default defineComponent({
  name: "TabelaNotasEmitir",
  setup() {
    const arraylistanotas = ref([])
    ...
    const arrayEmitir = async () => {
      try {
        const { data } = await api.get('v1/nota-fiscal/notas-por-emitir')
        // console.log(data)
        loading.value = false
        arraylistanotas.value = data
        if (data == 'Erro! nada encontrado') {
          arraylistanotas.value = []
        }
      } catch (error) {
