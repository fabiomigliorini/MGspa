...
<template>
 ...
 <q-separator></q-separator>
  ...
  <div class="col-md-3 items-start q-gutter-md text-left">
    ...
    <q-card
     class="col-md-3 bg-primary text-white" v-for="(listagempessoas, index) in listapessoas" v-bind:key="index">
      <q-card-section>
        <q-btn @click="urlviewpessoa(listagempessoas.codpessoa)" flat><div class="text-h5">{{ listagempessoas.fantasia }}</div></q-btn>
    ...
    <template v-slot:loading>
      ...
      <q-card>
        ...
        <q-card-section class="q-pt-none">
          <q-input label="CNPJ" mask="##.###.###/####-##" dense v-model="importarsefazmodel.cnpj" autofocus @keyup.enter="dialogimportar = false" unmasked-value/>
          <q-input label="CPF" mask="###.###.###-##" dense v-model="importarsefazmodel.cpf" @keyup.enter="dialogimportar = false" unmasked-value />
          <q-input label="IE" dense v-model="importarsefazmodel.ie" @keyup.enter="dialogimportar = false" />
          <q-select label="UF" dense v-model="importarsefazmodel.uf"
           :options="[
            {label:'Acre', value:'AC'},
          ...
          <q-select label="Filial" v-model="importarsefazmodel.codfilial" dense  autofocus @keyup.enter="dialogimportar = false" />
        </q-card-section>
