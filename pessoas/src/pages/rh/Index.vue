...
<template>
  ...
  <q-dialog v-model="dialogNovoPeriodo">
    <q-card bordered flat style="width: 500px; max-width: 90vw">
      <q-card-section class="text-grey-9 text-overline"> NOVO PERÍODO </q-card-section>
      ...
      <q-form @submit="criarPeriodo()">
        ...
        <q-card-actions align="right" class="text-primary">
          ...
          <q-btn flat label="Criar Período" type="submit" :loading="loadingNovoPeriodo" />
        </q-card-actions>
      </q-form>
    </q-card>
  </q-dialog>
  ...
  <MGLayout drawer>
    ...
    <template #drawer>
      <div class="q-pa-none q-pt-sm">
        ...
        <q-list separator v-if="sRh.periodos.length > 0">
          ...
          >
            <q-item-section>
              ...
              <q-item-label
                caption
                :class="periodoSelecionado(periodo.codperiodo) ? 'text-white' : ''"
              >
        ...
        <div v-else class="q-pa-md text-center text-grey">Nenhum período cadastrado</div>
      </div>
    </template>
