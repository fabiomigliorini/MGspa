...
const abrirNovaMeta = () => {
  ...
  if (ultima?.periodofinal) {
    ...
    if (partes) {
      const lastEnd = new Date(parseInt(partes[1]), parseInt(partes[2]) - 1, parseInt(partes[3]))
      const nextDay = new Date(lastEnd)
      nextDay.setDate(nextDay.getDate() + 1)
      pi = formatDTLocal(nextDay, 0, 0)

      const endDate = new Date(nextDay.getFullYear(), nextDay.getMonth() + 1, 25)
...
<template>
  ...
  <q-dialog v-model="dialogNovaMeta">
    <q-card bordered flat style="width: 500px; max-width: 90vw">
      <q-card-section class="text-grey-9 text-overline"> NOVA META </q-card-section>
      ...
      <q-form @submit="criarMeta()">
        ...
        <q-card-section>
          ...
          <div class="text-caption text-grey q-mt-sm">
            A configuracao (unidades, colaboradores, bonificacoes) sera copiada automaticamente da
            ultima meta.
          </div>
        </q-card-section>
        ...
        <q-card-actions align="right" class="text-primary">
          ...
          <q-btn flat label="Criar Meta" type="submit" :loading="loadingNovaMeta" />
        </q-card-actions>
      </q-form>
    </q-card>
  </q-dialog>
        ...
        <div style="max-width: 1100px; margin: auto" class="q-pa-md">
          <q-card bordered flat>
            <q-card-section class="text-grey-9 text-overline row items-center">
              METAS
              <q-space />
              <q-btn flat round dense icon="add" size="sm" color="primary" @click="abrirNovaMeta()">
                <q-tooltip>Nova Meta</q-tooltip>
              </q-btn>
            </q-card-section>

            <q-markup-table flat separator="horizontal" v-if="sMeta.listagem.length > 0">
              <thead>
                <tr class="text-left">
              ...
              <tbody>
                <tr v-for="meta in sMeta.listagem" :key="meta.codmeta">
                  <td>{{ formataData(meta.periodoinicial) }}</td>
                  <td>{{ formataData(meta.periodofinal) }}</td>
                  <td>
                    <q-badge :color="statusColor(meta.status)" :label="statusLabel(meta.status)" />
                  </td>
                  <td class="text-right">
                    <!-- DASHBOARD -->
                    <q-btn
            ...
            <div v-else class="q-pa-md text-center text-grey">Nenhuma meta cadastrada</div>
          </q-card>
        </div>
      </q-page>
    </template>
