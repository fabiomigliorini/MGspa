          ...
          <div class="q-pa-lg col-xs-12 col-sm-8 col-md-6 col-lg-4 col-xl-3">
            <q-stepper vertical v-model="step" color="primary" animated header-nav>
              ...
              <q-step :name="0" title="Tipo" :done="step > 0">
                <p>Selecione o Tipo de pessoa, Física para CPF ou Jurídica para CNPJ.</p>
              ...
              <q-step :name="2" title="Duplicidade" :done="step > 2">
                <template v-if="cadastrosEncontrados.length > 0">
                  <q-banner rounded inline-actions class="text-white bg-red q-mb-md">
                    Já existe cadastro com esse mesmo CNPJ/CPF. Verifique com atenção para evitar
                ...
                <template v-else>
                  Não existe nenhum cadastro com esse mesmo CNPJ/CPF, você pode prosseguir sem medo
                  de duplicar o cadastro!
                </template>
              </q-step>
              ...
              <q-step :name="3" title="Inscrição" :done="step > 3">
                <p>
                  Essas sãos as incrições estaduais que a SEFAZ retornou como válidas. Por favor
                  selecione uma ou clique em sem inscrição para não vincular o cadastro à nenhuma
                  delas.
                </p>
                  ...
                  >
                    ...
                    <q-item-section>
                      ...
                      <q-item-label v-if="ie.ender" caption>
                        ...
                        <template v-if="ie.ender.xCpl"> {{ ie.ender.xCpl }} - </template>
              ...
              <q-step :name="4" title="Finalizar " :done="step > 4">
                <MgInputFormatado
                  ...
                  maxlength="50"
                  @focus="fantasiaFocus"
                  @update:model-value="preencherRazaoSeVazia"
                />
  ...
  methods: {
    ...
    async sugerirNome() {
      ...
      if (this.sefazCadastro && this.sefazCadastro.length > 0) {
        ...
        if (insc) {
          this.model.pessoa = primeiraLetraMaiuscula(removerAcentos(insc.xNome).substr(0, 50))
          if (insc.xFant) {
            this.model.fantasia = primeiraLetraMaiuscula(removerAcentos(insc.xFant).substr(0, 50))
          } else {
            this.model.fantasia = this.model.pessoa
          }
          return
    ...
    async salvar() {
      ...
      if (this.model.ie) {
        consultas.sefaz = this.sefazCadastro.find((el) => {
          return el.IE == this.model.ie
        })
      }

      // parametros com o model e as consultas
      const params = Object.assign(this.model, consultas)
