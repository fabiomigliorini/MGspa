      ...
      <div class="q-pa-md">
        <q-card bordered>
          <q-table
            ...
            :rows="ferias"
            :columns="columns"
            no-data-label="Nenhuma programação de férias encontrada"
            :separator="separator"
            emit-value
            ...
            <template v-slot:body="ferias">
              <q-tr :props="ferias">
                ...
                <q-td
                  key="fantasia"
                  :props="ferias"
                  @click="linkPessoa(ferias.row.codpessoa)"
                ...
                <q-td
                  :key="this.ano == moment().year() ? 'janeiro' : 'dezembro'"
                  :colspan="this.ano == moment().year() ? '14' : '14'"
                  :props="ferias"
                >
                ...
                <q-td key="data" :props="ferias">
                  <div v-for="feriasS in ferias.row.ferias" v-bind:key="feriasS.codferias">
                    {{
                      formataDiaSemana(feriasS.gozoinicio) +
                      ', ' +
                      formataDataAbreviada(feriasS.gozoinicio, 0)
                    }}
                    <br />
                    a
...
export default {
  ...
  methods: {
    alteraRange(range, ferias) {
      ...
      const gozoinicio = moment()
        .startOf('year')
        .add(range.min - 1, 'days')
      const gozofim = moment()
        .startOf('year')
        .add(range.max - 1, 'days')
      ...
      ret.data.forEach((colab) => {
        colab.ferias.forEach((ferias) => {
          ...
          if (
            ...
            ferias.diagozoinicio > 335 &&
            ferias.diagozofim < 31
          ) {
