...
const buscarTitulos = debounce(async () => {
  ...
  try {
    ...
    $q.notify({
      ...
      textColor: 'white',
      icon: 'warning',
      message: error.response?.data?.message || 'Erro ao carregar títulos abertos',
    })
  }
}, 500)
...
const coresVencimento = (vencimento) => {
  if (vencimento >= formataDataIso(new Date())) return 'text-green'
  if (vencimento >= formataDataIso(moment().subtract(5, 'day').startOf('day').toDate()))
    return 'text-orange'
  return 'text-red'
}
...
const linkRelatorioTitulosAbertos = () => {
  return (
    process.env.API_URL + 'v1/titulo/relatorio-pdf?codgrupoeconomico=' + props.codgrupoeconomico
  )
}
    ...
    >
      ...
      <template v-slot:body="scope">
        <q-tr :props="scope" @click="linkMgSis(scope.row.codtitulo)">
          ...
          <q-td key="saldo" :props="scope" :class="coresSaldo(scope.row.saldo)">
            {{
          ...
          <q-td key="vencimento" :props="scope" :class="coresVencimento(scope.row.vencimento)">
