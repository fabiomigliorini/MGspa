...
function sugereAquisitivoFim() {
  ...
  model.value.aquisitivofim = inicio.add(1, 'year').subtract(1, 'day').format('YYYY-MM-DD')
  sugereGozo()
}
...
function calculaDiasGozo() {
  ...
  var diasgozo = model.value.dias - model.value.diasabono - model.value.diasdescontados
...
<template>
  <q-list v-if="colaborador.Ferias?.length > 0">
    <template v-for="ferias in feriasOrdenadas" v-bind:key="ferias.codferias">
      <q-separator inset />
      <q-item>
        ...
        <q-item-section>
          <q-item-label class="text-weight-bold">
            ...
            <q-badge v-if="ferias.prevista" color="orange" class="q-ml-sm"> Prevista </q-badge>
          </q-item-label>
          ...
          <q-item-label caption v-if="ferias.diasgozo">
            ...
            <span v-if="ferias.diasabono"> / {{ ferias.diasabono }} de Abono </span>
            <span v-if="ferias.diasdescontados"> / {{ ferias.diasdescontados }} Descontados </span>
            <span v-if="ferias.dias != ferias.diasgozo"> = {{ ferias.dias }} Total </span>
          </q-item-label>
        ...
        <q-item-section side>
          <q-item-label caption>
            <q-btn flat dense round icon="edit" size="sm" color="grey-7" @click="editar(ferias)">
              <q-tooltip>Editar</q-tooltip>
            </q-btn>
            <q-btn flat dense round icon="delete" size="sm" color="grey-7" @click="excluir(ferias)">
