...
const modalNovoEmail = async () => {
  ...
  const cobranca = sPessoa.item.PessoaEmailS.filter((email) => email.cobranca == true).length == 0
  const nfe = sPessoa.item.PessoaEmailS.filter((email) => email.nfe == true).length == 0
...
<template>
    ...
    <q-card bordered flat style="width: 600px; max-width: 90vw">
      ...
      <q-form @submit="submit()">
        ...
        <q-card-section>
          ...
          />
          <MgInputFormatado outlined v-model="modelEmail.apelido" label="Apelido" />
  ...
  <q-card bordered flat>
    ...
    <q-list v-if="emailsFiltrados.length > 0">
      <template v-for="(element, i) in emailsFiltrados" v-bind:key="element.codpessoaemail">
        <q-separator inset />
        <q-item>
          <!-- BOTAO EMAIL -->
          <q-item-section avatar>
            <q-btn round icon="email" color="primary" flat :href="linkEmail(element.email)" />
          </q-item-section>
    ...
    <div v-else class="q-pa-md text-center text-grey">Nenhum email cadastrado</div>
  </q-card>
</template>
