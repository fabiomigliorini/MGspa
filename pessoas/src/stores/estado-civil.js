...
export const estadoCivilStore = defineStore('estadoCivil', {
  ...
  actions: {
    ...
    async update(codestadocivil, model) {
      ...
      const index = this.estadosCivis.findIndex((e) => e.codestadocivil === codestadocivil)
      if (index !== -1) {
    ...
    async destroy(codestadocivil) {
      ...
      this.estadosCivis = this.estadosCivis.filter((e) => e.codestadocivil !== codestadocivil)
      return ret
    },

    async inativar(codestadocivil) {
      const ret = await api.post('v1/estado-civil/' + codestadocivil + '/inativo')
      const index = this.estadosCivis.findIndex((e) => e.codestadocivil === codestadocivil)
      if (index !== -1) {
        this.estadosCivis[index] = ret.data.data
      }
    ...
    async ativar(codestadocivil) {
      const ret = await api.delete('v1/estado-civil/' + codestadocivil + '/inativo')
      const index = this.estadosCivis.findIndex((e) => e.codestadocivil === codestadocivil)
      if (index !== -1) {
        this.estadosCivis[index] = ret.data.data
      }
