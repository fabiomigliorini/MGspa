  ...
  actions: {
    ...
    async telefoneVerificar(codpessoa, codpessoatelefone) {
      const ret = await api.get(
        'v1/pessoa/' + codpessoa + '/telefone/' + codpessoatelefone + '/verificar',
      )
      return ret
    },
    ...
    async novoHistoricoCobranca(codpessoa, historico) {
      const ret = await api.post('v1/pessoa/' + codpessoa + '/cobrancahistorico/', {
        historico: historico,
        codpessoa: codpessoa,
      })
      return ret
    ...
    async totaisNegocios(codgrupoeconomico, Negocios) {
      const ret = await api.get('v1/grupo-economico/' + codgrupoeconomico + '/totais-negocios', {
        params: Negocios,
      })
      return ret
    },
    ...
    async titulosAbertos(codgrupoeconomico, model) {
      const ret = await api.get('v1/grupo-economico/' + codgrupoeconomico + '/titulos-abertos', {
        params: model,
      })
      return ret
    },
    ...
    async nfeTerceiro(codgrupoeconomico, model) {
      const ret = await api.get('v1/grupo-economico/' + codgrupoeconomico + '/nfe-terceiro', {
        params: model,
      })
      return ret
    },
