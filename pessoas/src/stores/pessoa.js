  ...
  actions: {
    ...
    async transferirMercosId(codpessoa, mercosid, codpessoanova) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/transferir-mercos-id",
        {
          mercosid: mercosid,
    ...
    async getEndereco(codpessoa, codpessoaendereco) {
      const { data } = await api.get(
        "v1/pessoa/" + codpessoa + "/endereco/" + codpessoaendereco
      );
    ...
    async telefoneParaCima(codpessoa, codpessoatelefone) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/telefone/" + codpessoatelefone + "/cima"
      );
      return ret;
    },
    async telefoneParaBaixo(codpessoa, codpessoatelefone) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/telefone/" + codpessoatelefone + "/baixo"
      );
      return ret;
    },
    async telefoneAtivar(codpessoa, codpessoatelefone) {
      const ret = await api.delete(
        "v1/pessoa/" + codpessoa + "/telefone/" + codpessoatelefone + "/inativo"
      );
      return ret;
    },
    ...
    async telefoneAlterar(codpessoa, codpessoatelefone, model) {
      const ret = await api.put(
        "v1/pessoa/" + codpessoa + "/telefone/" + codpessoatelefone,
        model
      );
      const i = this.item.PessoaTelefoneS.findIndex(
    ...
    async telefoneConfirmaVerificacao(
      codpessoa,
      codpessoatelefone,
      codverificacao
    ) {
    ...
    async emailParaCima(codpessoa, codpessoaemail) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/email/" + codpessoaemail + "/cima"
      );
      return ret;
    },
    ...
    async emailParaBaixo(codpessoa, codpessoaemail) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/email/" + codpessoaemail + "/baixo"
      );
      return ret;
    },
    ...
    async emailExcluir(codpessoa, codpessoaemail) {
      const ret = await api.delete(
        "v1/pessoa/" + codpessoa + "/email/" + codpessoaemail
      );
      return ret;
    },
    ...
    async emailSalvar(codpessoa, codpessoaemail, modelalteraremail) {
      ...
      const i = this.item.PessoaEmailS.findIndex(
        (item) => item.codpessoaemail === codpessoaemail
      );
    ...
    async emailInativar(codpessoa, codpessoaemail) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/email/" + codpessoaemail + "/inativo"
      );
      const i = this.item.PessoaEmailS.findIndex(
        (item) => item.codpessoaemail === codpessoaemail
      );
    ...
    async emailVerificar(codpessoa, codpessoaemail) {
      const ret = await api.get(
        "v1/pessoa/" + codpessoa + "/email/" + codpessoaemail + "/verificar"
      );
      return ret;
    },
    ...
    async enderecoParaCima(codpessoa, codpessoaendereco) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/endereco/" + codpessoaendereco + "/cima"
      );
      return ret;
    },
    ...
    async enderecoParaBaixo(codpessoa, codpessoaendereco) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/endereco/" + codpessoaendereco + "/baixo"
      );
      return ret;
    },
    ...
    async enderecoInativar(codpessoa, codpessoaendereco) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/endereco/" + codpessoaendereco + "/inativo"
      );
      const i = this.item.PessoaEnderecoS.findIndex(
        (item) => item.codpessoaendereco === codpessoaendereco
      );
      this.item.PessoaEnderecoS[i] = ret.data.data;
      return ret;
    },
    ...
    async clienteSalvar(codpessoa, modelEditarCliente) {
      ...
      const i = this.arrPessoas.findIndex(
        (item) => item.codpessoa === codpessoa
      );
      this.arrPessoas[i] = ret.data.data;
      return ret;
    },
    ...
    async buscaFormaPagamento(codformapagamento) {
      const ret = await api.get(
        "v1/pessoa/formadepagamento?codformapagamento=" + codformapagamento
      );
      return ret;
    },
    ...
    async selectBanco(banco) {
      const ret = await api.get("v1/pessoa/conta/banco/select", {
        params: banco,
      });
      return ret;
    },
    ...
    async getCobrancaHistorico(codpessoa, page) {
      const ret = await api.get(
        "v1/pessoa/" + codpessoa + "/cobrancahistorico/",
        { params: page }
      );
      return ret;
    },
    ...
    async novoHistoricoCobranca(codpessoa, historico) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/cobrancahistorico/",
        { historico: historico, codpessoa: codpessoa }
      );
      return ret;
    },
    ...
    async deletaCobrancaHistorico(codpessoa, codcobrancahistorico) {
      const ret = await api.delete(
        "v1/pessoa/" + codpessoa + "/cobrancahistorico/" + codcobrancahistorico
      );
      return ret;
    },

    //TEM QUE POR O CODPESSOA
    async salvarHistoricoCobranca(
      codpessoa,
    ...
    async getRegistroSpc(codpessoa, page) {
      const ret = await api.get("v1/pessoa/" + codpessoa + "/registrospc/", {
        params: page,
      });
      return ret;
    },
    ...
    async novoRegistroSpc(codpessoa, modelnovoRegistroSpc) {
      const ret = await api.post(
        "v1/pessoa/" + codpessoa + "/registrospc/",
        modelnovoRegistroSpc
      );
      return ret;
    },

    async salvarEdicaoRegistro(
      codpessoa,
      codregistrospc,
      modelEdicaoRegistroSpc
    ) {
      const ret = await api.put(
        "v1/pessoa/" + codpessoa + "/registrospc/" + codregistrospc,
        modelEdicaoRegistroSpc
      );
      return ret;
    },
    ...
    async inativarCertidao(codpessoacertidao) {
      const ret = await api.post(
        "v1/certidao/" + codpessoacertidao + "/inativo"
      );
      return ret;
    },
    ...
    async totaisNegocios(codgrupoeconomico, Negocios) {
      const ret = await api.get(
        "v1/grupo-economico/" + codgrupoeconomico + "/totais-negocios",
        { params: Negocios }
      );
      return ret;
    },
    ...
    async titulosAbertos(codgrupoeconomico, model) {
      const ret = await api.get(
        "v1/grupo-economico/" + codgrupoeconomico + "/titulos-abertos",
        { params: model }
      );
      return ret;
    },
    ...
    async nfeTerceiro(codgrupoeconomico, model) {
      const ret = await api.get(
        "v1/grupo-economico/" + codgrupoeconomico + "/nfe-terceiro",
        { params: model }
      );
      return ret;
    },

    async salvarEdicaoContaBancaria(
      codpessoa,
      codpessoaconta,
      modelContaBancaria
    ) {
      const ret = await api.put(
        "v1/pessoa/" + codpessoa + "/conta/" + codpessoaconta,
        modelContaBancaria
      );
      return ret;
    },
    ...
    async contaBancariaInativar(codpessoaconta) {
      const ret = await api.post(
        "v1/pessoa/conta/" + codpessoaconta + "/inativo"
      );
      const i = this.item.PessoaContaS.findIndex(
        (item) => item.codpessoaconta === codpessoaconta
      );
