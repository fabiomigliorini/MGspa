...
export const unidadeNegocioStore = defineStore('unidadenegocio', {
  ...
  actions: {
    ...
    async atualizar(codunidadenegocio, model) {
      const ret = await api.put('v1/unidade-negocio/' + codunidadenegocio, model)
      return ret
    },

    async excluir(codunidadenegocio) {
      const ret = await api.delete('v1/unidade-negocio/' + codunidadenegocio)
      this.listagem = this.listagem.filter((u) => u.codunidadenegocio !== codunidadenegocio)
      return ret
    },

    async inativar(codunidadenegocio) {
      const ret = await api.post('v1/unidade-negocio/' + codunidadenegocio + '/inativo')
      const i = this.listagem.findIndex((u) => u.codunidadenegocio === codunidadenegocio)
