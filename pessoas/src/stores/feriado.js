...
export const feriadoStore = defineStore('feriado', {
  ...
  actions: {
    ...
    async excluir(codferiado) {
      ...
      this.listagem = this.listagem.filter((f) => f.codferiado !== codferiado)
      return ret
    },

    async inativar(codferiado) {
      const ret = await api.post('v1/feriado/' + codferiado + '/inativo')
      const i = this.listagem.findIndex((f) => f.codferiado === codferiado)
      if (i !== -1) this.listagem[i] = ret.data.data
      return ret
    },

    async ativar(codferiado) {
      const ret = await api.delete('v1/feriado/' + codferiado + '/inativo')
      const i = this.listagem.findIndex((f) => f.codferiado === codferiado)
      if (i !== -1) this.listagem[i] = ret.data.data
      return ret
    },
