...
export const tipoSetorStore = defineStore('tiposetor', {
  ...
  actions: {
    ...
    async excluir(codtiposetor) {
      ...
      this.listagem = this.listagem.filter((t) => t.codtiposetor !== codtiposetor)
      return ret
    },

    async inativar(codtiposetor) {
      const ret = await api.post('v1/tipo-setor/' + codtiposetor + '/inativo')
      const i = this.listagem.findIndex((t) => t.codtiposetor === codtiposetor)
      if (i !== -1) this.listagem[i] = ret.data.data
      return ret
    },

    async ativar(codtiposetor) {
      const ret = await api.delete('v1/tipo-setor/' + codtiposetor + '/inativo')
      const i = this.listagem.findIndex((t) => t.codtiposetor === codtiposetor)
      if (i !== -1) this.listagem[i] = ret.data.data
      return ret
    },
