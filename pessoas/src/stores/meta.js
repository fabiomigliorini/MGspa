...
export const metaStore = defineStore('meta', {
  ...
  actions: {
    ...
    async getDashboardColaborador(codmeta, codpessoa) {
      const ret = await api.get('v1/meta/' + codmeta + '/dashboard/' + codpessoa)
      this.dashboardColaborador = ret.data.data
      return ret
    },

    async getDashboardColaboradorEventos(codmeta, codpessoa, page = 1) {
      const ret = await api.get('v1/meta/' + codmeta + '/dashboard/' + codpessoa + '/eventos', {
        params: { page },
      })
      return ret
    },
    ...
    async atualizarUnidade(codmeta, codunidadenegocio, data) {
      const ret = await api.put('v1/meta/' + codmeta + '/unidade/' + codunidadenegocio, data)
      this.item = ret.data.data
      return ret
    },

    async removerUnidade(codmeta, codunidadenegocio) {
      const ret = await api.delete('v1/meta/' + codmeta + '/unidade/' + codunidadenegocio)
      this.item = ret.data.data
      return ret
    },
    ...
    async atualizarPessoa(codmeta, id, data) {
      const ret = await api.put('v1/meta/' + codmeta + '/pessoa/' + id, data)
      this.item = ret.data.data
      return ret
    },
    ...
    async criarFixo(codmeta, idPessoa, data) {
      const ret = await api.post('v1/meta/' + codmeta + '/pessoa/' + idPessoa + '/fixo', data)
      this.item = ret.data.data
      return ret
    },
