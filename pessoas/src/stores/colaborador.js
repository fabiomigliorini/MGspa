...
export const colaboradorStore = defineStore('colaborador', {
  ...
  actions: {
    ...
    async postFerias(model) {
      const ret = await api.post('v1/colaborador/' + model.codcolaborador + '/ferias', model)
      const i = this.colaboradores.findIndex((item) => item.codcolaborador === model.codcolaborador)
    ...
    async salvarColaboradorCargo(model) {
      const ret = await api.put(
        'v1/colaborador/' + model.codcolaborador + '/cargo/' + model.codcolaboradorcargo,
        model,
      )
    ...
    async salvarColaborador(modelEditColaborador) {
      const ret = await api.put(
        'v1/colaborador/' + modelEditColaborador.codcolaborador,
        modelEditColaborador,
      )
      return ret
    },
