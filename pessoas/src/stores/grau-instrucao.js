...
export const grauInstrucaoStore = defineStore('grauInstrucao', {
  ...
  actions: {
    ...
    async update(codgrauinstrucao, model) {
      ...
      const index = this.grausInstrucao.findIndex((g) => g.codgrauinstrucao === codgrauinstrucao)
      if (index !== -1) {
    ...
    async inativar(codgrauinstrucao) {
      const ret = await api.post('v1/grau-instrucao/' + codgrauinstrucao + '/inativo')
      const index = this.grausInstrucao.findIndex((g) => g.codgrauinstrucao === codgrauinstrucao)
      if (index !== -1) {
        this.grausInstrucao[index] = ret.data.data
      }
    ...
    async ativar(codgrauinstrucao) {
      const ret = await api.delete('v1/grau-instrucao/' + codgrauinstrucao + '/inativo')
      const index = this.grausInstrucao.findIndex((g) => g.codgrauinstrucao === codgrauinstrucao)
      if (index !== -1) {
        this.grausInstrucao[index] = ret.data.data
      }
