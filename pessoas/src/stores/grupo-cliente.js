...
export const grupoClienteStore = defineStore('grupoCliente', {
  ...
  actions: {
    ...
    async update(codgrupocliente, model) {
      ...
      const index = this.grupos.findIndex((g) => g.codgrupocliente === codgrupocliente)
      if (index !== -1) {
    ...
    async destroy(codgrupocliente) {
      ...
      this.grupos = this.grupos.filter((g) => g.codgrupocliente !== codgrupocliente)
      return ret
    },

    async inativar(codgrupocliente) {
      const ret = await api.post('v1/grupo-cliente/' + codgrupocliente + '/inativo')
      const index = this.grupos.findIndex((g) => g.codgrupocliente === codgrupocliente)
      if (index !== -1) {
        this.grupos[index] = ret.data.data
      }
    ...
    async ativar(codgrupocliente) {
      const ret = await api.delete('v1/grupo-cliente/' + codgrupocliente + '/inativo')
      const index = this.grupos.findIndex((g) => g.codgrupocliente === codgrupocliente)
      if (index !== -1) {
        this.grupos[index] = ret.data.data
      }
