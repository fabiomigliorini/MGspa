...
/*
select mov.codestoquemovimento, mov.codestoquemes, mov.saidaquantidade, mes.customedio, round((mov.saidaquantidade * mes.customedio), 2), mov.saidavalor, 'wget http://192.168.2.205/MGLara/estoque/calcula-custo-medio/' || cast(mov.codestoquemes as varchar)
...
-- TOTAL MOVIMENTO DIFERENTE DO TOTAL DO MES
with tot as (
	select 
		em.codestoquemes
		, sum(em.entradaquantidade) as entradaquantidade
		, sum(em.entradavalor) as entradavalor
		, sum(em.saidaquantidade) as saidaquantidade
		, sum(em.saidavalor) as saidavalor
	from tblestoquemovimento em
	group by em.codestoquemes
)
select 
	m.codestoquemes
	, m.entradaquantidade
	...
	, m.saidavalor
	, tot.saidavalor
	, 'wget http://192.168.2.205/MGLara/estoque/calcula-custo-medio/' || cast(m.codestoquemes as varchar) 
from tblestoquemes m
left join tot on (tot.codestoquemes = m.codestoquemes)
where coalesce(m.entradaquantidade, 0) != coalesce(tot.entradaquantidade, 0)
or coalesce(m.entradavalor, 0) != coalesce(tot.entradavalor, 0)
or coalesce(m.saidaquantidade, 0) != coalesce(tot.saidaquantidade, 0)
or coalesce(m.saidavalor, 0) != coalesce(tot.saidavalor, 0)
limit 50