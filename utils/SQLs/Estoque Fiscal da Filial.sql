...
-- Totais
select p.codproduto, p.produto, p.preco, el.estoquelocal, sum(es.saldoquantidade) as saldoquantidade, sum(es.saldovalor) as saldovalor
from tblproduto p
...
group by p.codproduto, p.produto, p.preco, el.estoquelocal
