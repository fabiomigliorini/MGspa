      ...
      <q-list v-if="data.length > 0">
        ...
        <q-infinite-scroll @load="loadMore" ref="infiniteScroll">
          ...
          <template v-for="item in data">
            ...
            <!-- Link para detalhes -->
            <!-- <q-item :to="'/pessoa/' + item.codpessoa" > -->
            <q-item  >

              <!-- Imagem -->
              <!-- <q-item-section avatar>
                <q-avatar icon="hourglass_empty" color="red"  text-color="white" v-if="(item.codpessoa == null)" />
              ...
              <q-item-section >
                ...
                <q-item-label caption>
                  <template v-if="item.fisica">
                    {{ formataCPF(item.cnpj) }}
                  </template>
                  <template v-else>
...
export default {
  ...
  methods: {
    ...
    loadData: debounce(function (concat, done) {
      // salva no Vuex filtro da pessoa
      this.$store.commit('filtroPessoa/updateFiltroPessoa', this.filter)
  ...
  created () {
    this.filter = this.$store.state.filtroPessoa
  }
