...
const props = defineProps({
  ...
  yearDigits: { type: Number, default: 4, validator: (v) => v === 2 || v === 4 },
...
//   const datePart = props.yearDigits === 2 ? 'DD/MM/AA' : 'DD/MM/AAAA'
//   if (!isTimestamp.value) return datePart
//   if (hasSeconds.value) return `${datePart} HH:mm:ss`
...
function parseIso(iso) {
  ...
  if (!m) return null
  return validatedDate(+m[1], +m[2], +m[3], m[4] ? +m[4] : 0, m[5] ? +m[5] : 0, m[6] ? +m[6] : 0)
}
...
function dateToDisplay(dt) {
  if (!dt) return ''
  const yStr = props.yearDigits === 2 ? pad(dt.getFullYear() % 100) : String(dt.getFullYear())
...
function parseDisplay(str) {
  if (!str) return null
  const m = str.match(/^(\d{2})\/(\d{2})\/(\d{2}|\d{4})(?:\s+(\d{2}):(\d{2})(?::(\d{2}))?)?\s*$/)
  if (!m) return null
  let y = +m[3]
  if (m[3].length === 2) y = expand2DigitYear(m[3])
  return validatedDate(y, +m[2], +m[1], m[4] ? +m[4] : 0, m[5] ? +m[5] : 0, m[6] ? +m[6] : 0)
}
...
function parsePasted(raw) {
  ...
  const brRe = /^(\d{1,2})\/(\d{1,2})\/(\d{2}|\d{4})(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?\s*$/
  m = raw.match(brRe)
  if (m) {
    let y = +m[3]
    if (m[3].length === 2) y = expand2DigitYear(m[3])
...
watch(
  ...
  async (visible) => {
    ...
    fieldRoot?.querySelector?.('.q-field__focusable-action')?.setAttribute('tabindex', '-1')
  },
  { immediate: true },
)

const qDateValue = computed(() => (lastValid.value ? dateToIso(lastValid.value) : null))

const hourValue = computed(() => (lastValid.value ? lastValid.value.getHours() : null))
const minuteValue = computed(() => (lastValid.value ? lastValid.value.getMinutes() : null))
const secondValue = computed(() => (lastValid.value ? lastValid.value.getSeconds() : null))
...
function onTimeKeydown(e, part, isFirst, isLast) {
  ...
  if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
    const current =
      part === 'h' ? hourValue.value : part === 'm' ? minuteValue.value : secondValue.value
